...
# in folder named "posterior_predictive_checks_bw"
...
posterior_predictive_checks <- 
  function(dataset_to_copy, calibration_sample = FALSE, calibration_prop, 
    ...
    if(!calibration_sample){
      ggsave(filename = "group_membership_chains.pdf", plot = chain_convergence, 
             path = paste0(path_to_figures_folder, "posterior_predictive_checks_bw/", 
                           "ADAMS_prior/"), 
    ...
    for(chain in 1:num_chains){
      ...
      ggplot(data = subset , aes(x = value)) + 
        ...
        # JZ: and here
        geom_vline(aes(xintercept = truth), color = "black", size = 1) +
    ...
    for(chain in 1:num_chains){
      for(dem_group in unique(synthetic_count_plot_data$group)){
        ...
        ggplot(data = subset , aes(x = value)) + 
          # JZ: updated color here
          geom_histogram(fill = "grey70", color = "grey70") + 
          theme_bw() + 
          xlab("Contingency Cell Count") + ylab("") + 
          facet_wrap(facets = vars(cell), ncol = 2, scales = "free") +
          geom_vline(aes(xintercept = truth), 
                     color = "black", # JZ: and here
    ...
    for(chain_num in seq(1:num_chains)){
      ...
      ggplot(data = subset , aes(x = value)) + 
        ...
        geom_vline(aes(xintercept = truth), color = "black", size = 2) + # JZ: and here
      ...
      if(!calibration_sample){
        ggsave(filename = paste0(path_to_figures_folder, 
                                 ...
                                 "/continuous_vars/median/overall.pdf"), 
        ...
        for(var_name in unique(subset$label)){
          ...
          if(!calibration_sample){
            ggsave(filename = paste0(path_to_figures_folder, 
                                     ...
                                     tolower(dem_group), "/", var_name, ".pdf"), 
    ...
    for(chain_num in seq(1:num_chains)){
      ...
      ggplot(data = subset , aes(x = value)) + 
        ...
        xlab("Skew") + 
        facet_wrap(facets = vars(label), ncol = 2, scales = "free") +
        geom_vline(aes(xintercept = truth), color = "black" , size = 2) + 
        theme(panel.grid.minor = element_blank(),
              panel.grid.major = element_blank()) 
      ...
      if(!calibration_sample){
        ggsave(filename = paste0(path_to_figures_folder, 
                                 "posterior_predictive_checks_bw/", 
                                 "ADAMS_prior/run_", chain_num, 
                                 "/continuous_vars/skew/overall.pdf"), 
               width = 8, height = 10, units = "in")  
      } else{
        ggsave(filename = paste0(path_to_figures_folder, 
                                 "posterior_predictive_checks_bw/", 
    ...
    if(!calibration_sample){
      ggsave(filename = paste0(path_to_figures_folder, 
                               "posterior_predictive_checks_bw/", 
                               "ADAMS_prior/", 
                               "impairment_classes_combined_all_runs_count.pdf"), 
    ...
    if(!calibration_sample){
      ggsave(filename = paste0(path_to_figures_folder, 
                               "posterior_predictive_checks_bw/", 
                               "ADAMS_prior/", 
                               "impairment_classes_combined_all_runs_prop.pdf"), 
             height = 5, width = 10, units = "in")
    } else{
      ggsave(filename = paste0(path_to_figures_folder, 
                               "posterior_predictive_checks_bw/",
