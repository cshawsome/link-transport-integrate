...
#$ -j y 
#$ -l h_rt=17:00:00,h_data=4G #requests 17 hours, 4GB of data (per core) 
...
R CMD BATCH --no-save --no-restore "--args scenario=$SGE_TASK_ID save=\"no\" sens=\"no\" "  mask_impute_pool.R output/output.$JOB_ID.$SGE_TASK_ID
echo R CMD BATCH --no-save --no-restore \'--args scenario=$SGE_TASK_ID save=\"no\" sens=\"no\" \'  mask_impute_pool.R output/output.$JOB_ID.$SGE_TASK_ID
